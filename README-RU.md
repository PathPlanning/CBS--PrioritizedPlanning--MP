...
- low_level - алгоритм, используемый в поиске нижнего уровня в алгоритмах CBS, ECBS и Prioritized planning. Может принимать следующие значения:
    ...
    3. 2k_neigh - версия  алгоритма SIPP для [2<sup>k</sup>-связной сетки](https://jair.org/index.php/jair/article/download/11383/26555/). При этом агенты движутся с постоянной скоростью, и время прохождения ребра равно его длине, переведенной в целое количество тактов времени в соотвествии с параметром time_resolution. Также считается, что если агент частично находится в клетке, он накрывает ее целиком
    4. zero_scipp - субоптимальная версия SIPP, описанная [здесь](https://docs.google.com/document/d/16NjWHubNFczPDGi3QqWkMrWE8ewQtOBXruUBaa9hQnw/edit). Может работать в двух режимах в зависимости от значения параметра gen_subopt_from_opt
    5. focal_search - Алгритм Focal search как в оригинальной [статье](https://www.aaai.org/ocs/index.php/SOCS/SOCS14/paper/viewFile/8911/8875) про ECBS. В алгоритме используется вторичная эвристика, равная количеству вершинных конфликтов на уже построенном участке траектории до текущей вершины
    6. scipp - алгоритм [SCIPP](https://www.aaai.org/ocs/index.php/SOCS/SOCS19/paper/viewFile/18327/17443) (дискретная версия для четырехсвязной сетки)
...
- neigh_degree - степень связности для алгоритма SIPP с 2<sup>k</sup>-связной сеткой (то есть параметр k). Целое число больше либо равное 2, учитывается если выбран алгоритм CBS или ECBS. Опциональный параметр, значение по умолчанию равно 2
- time_resolution - параметр, используемый для переведения времени прохождения ребер в целое количество тактов времени в алгоритме SIPP с 2<sup>k</sup>-связной сеткой. Время прохождения ребра получается умножением его длины на time_resolution с последующим округлением вниз. Например, если time_resolution = 1000, переход в соседнюю ячейку по горизонтали занимает 1000 тактов, а по диагонали 1414 такта. Опциональный параметр, значение по умолчанию равно 1000
