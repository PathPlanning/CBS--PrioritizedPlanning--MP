...
struct SIPPNode : virtual public Node
{
    ...
    SIPPNode(int x = 0, int y = 0, Node *p = nullptr, int g_ = 0, double H_ = 0,
             int ConflictsCount = 0, int PrimitiveId = -1, int hc_ = 0) :
        Node(x, y, p, g_, H_, ConflictsCount, PrimitiveId), startTime(g_), endTime(g_) {}
    ...
    bool operator< (const SIPPNode &other) const {
        return std::tuple<int, int, int, int, int>(F, -g, -startTime, j, i) <
                std::tuple<int, int, int, int, int>(other.F, -other.g, -other.startTime, other.j, other.i);
    }
